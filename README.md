...
* **Loop Skewing / Permutation**: Demonstrated in the shifted loop implementations, rewriting the nested loops transforms the triangular iteration space into a rectangular one. This eliminates data dependencies across the inner loop and sets the stage for easy multi-threading.
* **Data-Level Parallelism (SIMD)**: Inner loops and utility functions (`vec_add`, `vec_sub`, `vec_zero`) aggressively utilize the `#pragma omp simd` directive to take advantage of the CPU's vector registers, processing multiple array elements in a single clock cycle.
...
* **Cache Tiling (Blocking)**: The tiled implementation collapses the outer loops and breaks memory access into localized blocks. This strategy drastically reduces cache misses, a critical bottleneck in memory-bound algorithms like convolution.
* **Task-Based Divide and Conquer**: For the Karatsuba algorithm, standard loop parallelization is insufficient. Instead, OpenMP's task-based parallelism (`#pragma omp task` and `#pragma omp taskwait`) is deployed to spawn parallel threads for the recursive, independent multiplications of the array halves.